...
// 递归实现反序列化为JS对象
const parseObject1 = (data) => {
    if (typeof data === 'object' && data!== null) {
        if (Array.isArray(data)) {
            // 如果 data 是数组，递归处理数组中的每个元素
            return data.map(item => parseObject(item));
        } else {
            // 如果 data 是对象，递归处理对象的每个属性
            const result = {};
            for (const [key, value] of Object.entries(data)) {
                if (typeof value ==='string' && (value.includes('{"') || value.includes("["))) {
                    try {
                        const parsedValue = JSON.parse(value);
                        result[key] = parseObject(parsedValue);
                    } catch (error) {
                        result[key] = value;
                    }
                } else {
...
//模拟响应数据
// const resp1 = "{\"person\": {\"name\": \"John Doe\", \"age\": 30, \"address\": {\"street\": \"123 Main St\", \"city\": \"Anytown\", \"state\": \"CA\", \"zip\": \"12345\"}}, \"hobbies\": [\"reading\", \"swimming\", \"running\"], \"friends\": [{\"name\": \"Jane Smith\", \"age\": 28}, {\"name\": \"Bob Johnson\", \"age\": 32}]}"
// // console.log(typeof resp1);
// const obj = JSON.parse(resp1);
// const obj1 = parseObject(obj);
// console.log("obj",obj);
...
const str = '{"info": "Some nested JSON data"}';
